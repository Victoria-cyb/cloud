...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const graphqlRequest = async function sendGraphQLRequest(query, variables = {}) {
    const API_URL = window.location.hostname.includes('localhost') || window.location.hostname.includes('127.0.0.1')
      ? 'http://localhost:4000/graphql'
      : 'https://cloudscrap-back.vercel.app/graphql'; // Verify this URL

    console.log("Sending GraphQL request:", JSON.stringify({ query, variables }, null, 2));
  ...
  // Hamburger menu and auth links (unchanged)
  const hamburger = document.querySelector('.hamburger');
  const navMenu = document.querySelector('.nav-menu');
  if (hamburger && navMenu) {
    hamburger.addEventListener('click', () => {
  ...
  // Only run scrape-related code on body.html
  if (window.location.pathname.includes('body.html')) {
    const scrapeRedditButton = document.querySelector('.scrape[data-platform="reddit"]');
    const emailModal = document.getElementById('email-modal');
    const closeModal = document.querySelector('.close-modal');
    const emailTableBody = document.getElementById('email-table-body');
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        try {
          ...
          const tableRows = emailTableBody.querySelectorAll('tr');
          if (tableRows.length === 0 || tableRows[0].textContent.includes('No emails found')) {
            console.log('No email data to download');
          } else {
            tableRows.forEach(row => {
              const cells = row.querySelectorAll('td');
              rows.push([
  ...
  // Login and signup forms (unchanged)
  const loginForm = document.getElementById('login-form');
  const signupForm = document.getElementById('signup-form');
  const resultDiv = document.getElementById('result');
